...
class TrafficDataset(Dataset):
    def __init__(self, csv_file, seq_length=24, knowledge_dim=3, feature_cols=None, target_col="OT", transform=None):
        ...
        # 将特征和目标列转换为数值类型；无法转换的置为 NaN，然后删除含 NaN 的行
        ...
        # 计算知识向量（使用归一化后的目标值统计信息：均值、标准差、最大值）
        stats = []
        if knowledge_dim >= 1:
...
def get_dataloader(csv_file, batch_size=32, seq_length=24, knowledge_dim=3, feature_cols=None, target_col="OT", shuffle=True):
    dataset = TrafficDataset(csv_file, seq_length, knowledge_dim, feature_cols, target_col)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)
    return dataloader

if __name__ == '__main__':
    csv_file = "/root/autodl-tmp/fusion/dataset/traffic.csv"
